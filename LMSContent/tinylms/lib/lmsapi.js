...
function LMSAPI_readCMIdataModel(itemElement) {
	...
	if (resource != null) {
	// modifica 16-04-2014	this.setValue("cmi.core.lesson_status", resource.cmi_core_lesson_status);
	    this.setValue("cmi.core.lesson_status", (resource.cmi_core_lesson_status == "") ? "ab-initio" : "");
...
function LMSAPI_resetDatabase() {
    ...
    if (this.cam == null) {
        alert("Errore nel caricamento dei file" + API.coursePath);
    }
  ...
  var node = this.cam.resources.getFirstLeaf();
	while (node != null && node != this.cam.resources) {
	  if (node.isResourceElement) {
			...
			node.cmi_core_sraw = ""; // "cmi.core.score.raw" is used by lmsapi.js
			node.cmi_core_total_time = "00:00.00"; // "cmi.core.total_time" is used by lmsapi.js
...
function LMSAPI_LMSFinish(anEmptyString) {
  ...
  this.WS.Finish(API.UserID, API.Reference,API.IDEnter,API.CourseID, paramStr);
...
function normalize(time) {
    ...
    var str = zero4.pad(a[0]) + ":" + a[1]+ ":" + a[2] + ".00";
    return str;
	...
	for (var i=0; i < this.dataModel.length; i++) {
		...
		if (this.dataModel[i].key == key) {
			...
			// Special treatment for the CMITimespan type HHHH:MM:SS.SS	 
			    ...
			    var totaltime;
			    if (this.getValue("cmi.core.total_time").length != 13) {
			        totaltime = normalize(this.getValue("cmi.core.total_time"));
			    } else {
			        totaltime = this.getValue("cmi.core.total_time")
			    }
			    var sessionTime = normalize(this.getValue("cmi.core.session_time"));

			    if (sessionTime != null && sessionTime.length == 13) {
			        this.setValue("cmi.core.total_time",
                        this.addTimespan(totaltime, sessionTime)
                    );
			    }
...
function KeepAliveSession() {
    $.post(location.origin + '/KeepAliveSession.ashx', function (data, status) {
...
function LMSAPI_gotoFirstItem() {
  ...
  if (this.getValue("cmi.core.student_id") == null) {
		...
		if (node == null) {
		  ...
		  window.lmsContentFrame.location.href=this.CoursewarePath+this.coursePath+node.getHRef();
		}
		//this.fireUpdateTOC();
	}
}
	  ...
	  this.anticipatedItem = itemElement;
		if (itemElement.parameters != null) {
		  if (itemElement.parameters.length > 0 && itemElement.parameters.charAt(0) == '?') {
		 		window.lmsContentFrame.location.href=this.CoursewarePath+this.coursePath+itemElement.getHRef()+itemElement.parameters;
			} else {
		 		window.lmsContentFrame.location.href=this.CoursewarePath+this.coursePath+itemElement.getHRef()+'?'+itemElement.parameters;
			}
		} else {
	  	window.lmsContentFrame.location.href=itemElement.getHRef();
		}
		//this.fireUpdateTOC();
	}
}
...
function LMSAPI() {
    ...
    new LMSDataModelEntry("cmi.core.lesson_location", "rw", "CMIString255", "", ""),
	...
	this.readCMIdataModel = LMSAPI_readCMIdataModelLMSLite;
	this.writeCMIdataModel = LMSAPI_writeCMIdataModelLMSLite;
	...
	//this.loadDatabase = LMSAPI_loadDatabaseFromCookie;
	//this.saveDatabase = LMSAPI_saveDatabaseToCookie;
	this.loadDatabase = LMSAPI_loadDatabaseFromLMSlite;
	this.saveDatabase = LMSAPI_saveDatabaseToLMSlite;
	//this.loadDatabase = LMSAPI_loadDatabaseFromParentLMS;
	//this.saveDatabase = LMSAPI_saveDatabaseToParentLMS;
	...
	this.Reference = 0;
	this.IDenter = 0;
	// Below is special Code for the Server Based version
	this.CoursewarePath = "";
	this.CourseID = 0; //Holds the CourseID used in the server based version
